...
```{r include=T}
###import data
pilotb <- read.csv("pilotb.csv", na.strings = c("", "NA"))
View(pilotb)
```

```{r include=T}
###create anonymized shareable dataset
pilotb_anonymized <- subset(pilotb, select = -c(4, 9:16)) # anonymize data
pilotb_anonymized <- pilotb_anonymized[-c(1:22), ] # delete pre-runs that are not pilot b
write.csv(pilotb_anonymized, "/Users/ericneumann/Desktop/stavrova2020/writeup/pilotb_anonymized.csv") # save for upload to github
```

```{r include=T}
###exclude all who failed attention check
pilotb_excl <- pilotb_anonymized %>%
  filter(att_check == "4")
View(pilotb_excl) # all passed attention check
```

```{r include=T}
###create aggregate cynicism score
pilotb_excl[, 13:21] <- sapply(pilotb_excl[, 13:21], FUN = as.numeric)
pilotb_excl$cyn_total <- (pilotb_excl$cyn1 + pilotb_excl$cyn2 + pilotb_excl$cyn3 + 
                                       pilotb_excl$cyn4 + pilotb_excl$cyn5 + pilotb_excl$cyn6 + 
                                       pilotb_excl$cyn7 + pilotb_excl$cyn8 + pilotb_excl$cyn9)/9
summary(pilotb_excl$cyn_total)
```

```{r include=T}
###separate by condition
pilotb_excl$disrespect <- ifelse(pilotb_excl$cond_disr == "NA", "guilt", "disrespect") # it shows no "guilt" label (instead says "NA"), so I created two columns instead
pilotb_excl$guilt <- ifelse(pilotb_excl$cond_guilt == "NA", "disrespect", "guilt")
cyn_means <- pilotb_excl %>%
  group_by(disrespect) %>%
...
pilotb_disrespect <- filter(pilotb_excl, pilotb_excl$disrespect == "disrespect")
pilotb_guilt <- filter(pilotb_excl, pilotb_excl$guilt == "guilt")
#t.test(pilotb_disrespect$cyn_total, pilotb_guilt$cyn_total, paired = F) # not enough observations in pilot b
```
